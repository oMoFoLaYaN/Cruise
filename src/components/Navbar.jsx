...
return (
  ...
  <nav className={`h-12 md:h-[4.5rem] flex justify-between items-center ${layout.container}`}>
    <a href="#" className={`font-bold text-lg text-${color} dark:text-water-100`}>
      Cruise
    </a>
            ...
            {navLinks.map((nav, index) => (
              <li
                ...
                className={`font-poppins font-medium capitalize cursor-pointer text-base ${
                  active === nav.title ? `text-water-700 dark:text-water-100 font-semibold underline decoration-water-700  
                    decoration-double decoration-1 underline-offset-8 duration-700 dark:decoration-water-100` : `text-water-700 dark:text-water-100`
